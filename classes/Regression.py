...
class Regression():
    ...
    def train(self,input_set,zone,algo_list=[1],hidden_layer_size=(4),n_jobs=1,kernel='rbf',n_components=15,n_estimators=50,loss='linear',learning_rate=1.0):
        ...
        x_train, x_test, y_train, y_test,scaler =self.read_dataset(input_set,n_components)
        for num in algo_list:         
            if num==1:
                ...
                return clf,zone,clf.score(x_test,y_test),'Linear Regression',scaler
            elif num==2:
                # SVR sigmoid
                clf=svm.SVR(kernel=kernel)
                clf.fit(x_train,y_train)
                # storeObj(clf, zone, clf.score(x_test, y_test), 'SVR'+','+kernel)
                return clf,zone,clf.score(x_test,y_test),'SVR'+kernel,scaler
            elif num==3:
                #Neural Net
                ...
                return clf,zone,clf.score(x_test,y_test),'NeuralNet hidden_size'+str,scaler
            elif num==4:
                ...
                return clf,zone,clf.score(x_test,y_test),'Gradient Boosted Regressor',scaler
            elif num==5:
                clf=ABR()
                clf.fit(x_train,y_train)
                # storeObj(clf, zone, clf.score(x_test, y_test), 'AdaBoost Regressor')
                return clf,zone,clf.score(x_test,y_test),'AdaBoost Regressor',scaler

    def test(self,model):
        pass

    @staticmethod
    def save_model(self,obj,zone,score,preobj,name):
        du=dumps(obj)
        fd.storeObj(pickleobj=du,zone=zone,acc=score,preobj=preobj,name=name)
